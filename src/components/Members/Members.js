...
const Members=(props)=>{
    ...
    const DATA=[
      ...
      let nonAssistants = DATA.filter((person) => !person.assistant);
      ...
      return (
        ...
        <div >
          {assistants.map((member) => (
            <Card key={member.id} name={member.name} group={member.group} assistant={String(member.assistant)}/>
          ))}
        </div>
        ...
        DATA.filter((student)=>student.group === groupNames && !student.assistant)
           .map(student=><Card key={student.id} name={student.name} group={student.group} assistant={student.assistant}/>
            )
          }
